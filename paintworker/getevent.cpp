...
static int open_device(const char* device)
{
    ...
    ALOGD("skip devices 0name =%s\n",name);
    if(!strstr(name,"hid-over-i2c 2D1F:0165")) {
        ALOGD("skip devices name =%s\n",name);
        close(fd);
        return -1;
    }
...
{
    fprintf(stderr, "read_notify dirname=%s\n",dirname);
    int res;
    ...
    while (res >= (int)sizeof(*event)) {
        ...
        if (event->len) {
            ...
            if (event->mask & IN_CREATE) {
                fprintf(stderr, "read_notify:open_device %s",devname);
                open_device(devname);
            } else {
...
{
    fprintf(stderr, "scan_dir:dirname=%s\n",dirname);
    char devname[PATH_MAX];
    char* filename;
    DIR* dir;
    struct dirent* de;
...
{
    fprintf(stderr, "uninit_getevent\n");
    int i;
    for (i = 0; i < nfds; i++) {
        close(ufds[i].fd);
    }
...
{
//	fprintf(stderr, "get_event timeout=%d",timeout);
    int res;
    int i;
    int pollres;
            ...
            if (ufds[i].revents) {
                if (ufds[i].revents & POLLIN) {
                    ...
                    if (event->type == EV_ABS && event->value != 0) {
                        if (ioctl(ufds[i].fd, EVIOCGABS(0x0035), &abs) == 0) {
                            //  printf(" : value %d, min %d, max %d, fuzz %d, flat %d, resolution %d",
                            //	  abs.value, abs.minimum, abs.maximum, abs.fuzz, abs.flat,abs.resolution);
                        }
                    }
